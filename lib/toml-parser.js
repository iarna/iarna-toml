...
function makeParserClass (Parser) {
  class TOMLParser extends Parser {
    ...
    parseDateTime () {
      ...
      this.state.year = this.state.buf
      this.state.buf = ''
    ...
    parseDateMonth () {
      if (this.char === '-') {
        ...
        this.state.month = this.state.buf
        this.state.buf = ''
    ...
    parseDateDay () {
      if (this.char === 'T') {
        ...
        this.state.date = `${this.state.year}-${this.state.month}-${this.state.buf}`
        this.state.buf = ''
    ...
    parseTimeHour () {
      if (this.char === ':') {
        ...
        this.state.hour = this.state.buf
        this.state.buf = ''
    ...
    parseTimeSec () {
      if (isDigit(this.char)) {
        this.consume()
        if (this.state.buf.length === 2) {
          this.state.time = `${this.state.hour}:${this.state.min}:${this.state.buf}`
          this.state.buf = ''
    ...
    parseTimeZoneOrFraction () {
      if (this.char === '.') {
        ...
        this.consume()
        return this.return(createDateTime(this.state.date + 'T' + this.state.time + this.state.buf))
      } else {
    ...
    parseTimeZoneMin () {
      if (isDigit(this.char)) {
        this.consume()
        if (/\d\d$/.test(this.state.buf)) return this.return(createDateTime(this.state.date + 'T' + this.state.time + this.state.buf))
      } else {
        throw this.error(new Error('Unexpected character in datetime, expected digit'))
      }
    }
